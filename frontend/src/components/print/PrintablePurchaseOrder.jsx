...
const PrintablePurchaseOrder = forwardRef(({ po, companyInfo }, ref) => {
    ...
    const company = {
        name: companyInfo?.companyName || companyInfo?.name || 'ALUECO ALUMINIUM SYSTEMS',
        address: companyInfo?.companyAddress || companyInfo?.address || '123 Industrial Zone, Colombo, Sri Lanka',
        taxNumber: companyInfo?.taxNumber || companyInfo?.companyTaxNumber || 'VAT-123456789',
        phone: companyInfo?.companyPhone || companyInfo?.phone || '+94 11 234 5678',
        email: companyInfo?.companyEmail || companyInfo?.email || 'info@alueco.lk',
    };
    ...
    return (
        <div ref={ref} className="print-container bg-white text-slate-900 p-8 max-w-[820px] mx-auto font-sans text-xs">
            {/* Header Banner */}
            <div className="flex justify-between items-start pb-4 border-b-2 border-indigo-950 mb-6">
                ...
                <div className="text-right">
                    ...
                    <p className="text-slate-500 text-[10px] mt-0.5">Date: {formatDate(po.poDate || po.createdAt)}</p>
                    <p className="text-slate-600 text-[10px] font-semibold uppercase mt-0.5">Status: {(po.status || 'draft').replace('_', ' ')}</p>
                </div>
            </div>

            {/* Supplier & Delivery Info Boxes */}
            <div className="grid grid-cols-2 gap-6 mb-6 bg-slate-50 p-4 rounded-xl border border-slate-200">
                <div>
                    ...
                    <p className="font-bold text-sm text-slate-900">{supplier.name || supplier.displayName || 'Not Specified'}</p>
                    {supplier.code && <p className="text-slate-600 font-mono text-[11px]">{supplier.code}</p>}
                    {supplier.taxRegistrationNumber && <p className="text-slate-600 text-[11px]">VAT: {supplier.taxRegistrationNumber}</p>}
                    {supplier.phone && <p className="text-slate-600 text-[11px]">Tel: {supplier.phone}</p>}
                </div>

                <div>
                    <p className="text-[10px] font-bold uppercase text-slate-400 tracking-wider mb-1">Ship To / Warehouse</p>
                    <p className="font-bold text-sm text-slate-900">{deliverTo.warehouseName || 'Main Warehouse'}</p>
                    {deliverTo.address?.line1 && (
                        <p className="text-slate-600 text-[11px]">
                            {deliverTo.address.line1}{deliverTo.address.city && `, ${deliverTo.address.city}`}
                        </p>
                    )}
                    ...
                    {po.paymentTerms?.type && (
                        <p className="text-slate-600 text-[11px]">
                            Payment Terms: <strong className="text-slate-800 uppercase">{po.paymentTerms.type}</strong>
                        </p>
                    )}
                </div>
            </div>
            ...
            <table className="w-full mb-6 border border-slate-200 rounded-lg overflow-hidden">
                <thead className="bg-slate-100 border-b border-slate-200 text-[10px] uppercase font-bold text-slate-600">
                    <tr>
                        ...
                        <th className="p-2.5 text-right w-24">Ordered Qty</th>
            ...
            <div className="flex justify-end mb-6">
                <div className="w-72 space-y-1.5 text-xs bg-slate-50 p-3 rounded-lg border border-slate-200">
                    ...
                    {po.shippingCost > 0 && <div className="flex justify-between text-slate-600"><span>Shipping:</span><span>{fmt(po.shippingCost)}</span></div>}
                    {po.otherCharges > 0 && <div className="flex justify-between text-slate-600"><span>Other Charges:</span><span>{fmt(po.otherCharges)}</span></div>}
            ...
            <div className="mt-6 pt-4 border-t border-slate-200 text-slate-700 text-[10px]">
                <p className="font-bold uppercase text-slate-900 mb-1.5">Procurement Terms &amp; Conditions</p>
                <ol className="list-decimal pl-4 space-y-0.5 text-slate-600">
                    <li>Goods supplied must conform strictly to the technical specifications and quantities detailed above.</li>
                    <li>All delivery notes and invoices must clearly reference this Purchase Order Number.</li>
                    <li>Goods will undergo physical inspection and QA acceptance upon receipt at the destination warehouse.</li>
                    <li>Any non-conforming or substandard materials will be rejected and returned at vendor's cost.</li>
                </ol>
            </div>
            ...
            <div className="mt-8 pt-6 border-t border-slate-200 flex justify-between items-end text-[10px] text-slate-500">
                <div>
                    <p className="font-bold text-slate-700">Issued by {company.name}</p>
                    <p>Standard Procurement Order — Valid with authorized electronic or physical signature.</p>
                </div>
